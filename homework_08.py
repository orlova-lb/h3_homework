...
class IpHandler:
    ...
    def reverse_IP(self):
        ...
        return [self.reversed_ip(ip) for ip in self.ipList]

    def get_oct_1_3(self):
        """Returns a list of IPs without first octets (127.0.0.1 -> .0.0.1)"""
        return [self.get_octet_1_3(ip) for ip in self.ipList]

    def get_oct_3(self):
        """Returns a list of last octets of each IP (127.0.0.1 -> .1)"""
        return [self.get_octet_3(ip) for ip in self.ipList]

    def reversed_ip(self, ip):
        reversed_ip = ip.split(".")
        reversed_ip.reverse()
        return ".".join(reversed_ip)

    def get_octet_1_3(self, ip):
        return ".".join(ip.split(".")[1:])

    def get_octet_3(self, ip):
        return ip.split(".")[3]
